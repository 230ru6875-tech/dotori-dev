...
function normalizeMarketLabels(){
  for(const [ticker,label] of Object.entries(MARKET_LABELS)){
    const card=marketCard(ticker);
    if(card)replaceExactText(card,ticker,label);
  }
}
...
const boot=()=>{normalizeMarketLabels();enhance();setInterval(enhance,60000)};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',boot,{once:true}):boot();
})();
</script>`;
...
console.log('Applied Korean Market Pulse display names without changing market values.');
